...
export class Game {
  ...
  get score() {
    return this.answers.filter((a) => a.success).length;
  }
  ...
  trySolution(solution) {
    ...
    if (solution === this.currentProblem.solution) {
      ...
      this.answers.push(
        new Answer(this.currentProblem, elapsed, firstTry, expired)
      );
      Event.fire(Events.ScoreChanged);
  ...
  getSuggestedSolutions() {
    ...
    let solutionIndex = this.allPossibleSolutions.indexOf(
      this._currentProblem.solution
    );
