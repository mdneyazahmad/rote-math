...
export class Problem {
  ...
  constructor(type, left, right) {
    ...
    if (type === ProblemType.Multiplication && (left === 0 || right === 0)) {
      ...
      if (left !== 0) {
        this.problemMode = coinToss(
          ProblemMode.HideRight,
          ProblemMode.HideAnswer
        );
      } else if (right !== 0) {
        this.problemMode = coinToss(
          ProblemMode.HideLeft,
          ProblemMode.HideAnswer
        );
      } else {
        // zero times zero.
  ...
  static makeProblems(args) {
    if (args.gameMode === GameMode.Competitive) {
      return range2(args.max + 1).map(
        (p) => new Problem(args.problemType, p.x, p.y)
      );
    } else {
      return range(args.max + 1).map(
        (x) => new Problem(args.problemType, args.practiceDigit, x)
      );
    }
  }
